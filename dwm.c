...
struct Pertag {
  ...
  unsigned int tagviews[LENGTH(tags) + 1];
...
createmon(void)
{
  ...
  for (i = 0; i <= LENGTH(tags); i++) {
    m->pertag->tagviews[i] = (1 << (i - 1)); // tag i 預設只記自己
  }
  m->pertag->tagviews[0] = TAGMASK; // curtag=0（全部 tags）時的預設視圖
...
#if SMART_TAGVIEW
    selmon->pertag->tagviews[selmon->pertag->curtag] = newtagset;
#endif
	...
	if (arg->ui & TAGMASK) {
		...
		if (arg->ui == ~0){
		  ...
		  selmon->tagset[selmon->seltags] = arg->ui & TAGMASK;
			...
			selmon->pertag->curtag = i + 1;
#if SMART_TAGVIEW
      selmon->tagset[selmon->seltags] = selmon->pertag->tagviews[selmon->pertag->curtag];
#else
      selmon->tagset[selmon->seltags] = arg->ui & TAGMASK;
#endif
		}
